...
public class PlayerDAO extends DAO<Player>{
    ...
    public Player find(Integer id) {
        ...
        try{
            ...
            if (result.first()){
                player4 = new Player(id,result.getString("username"),result.getString("email"),result.getString("password"),result.getInt("permission"));
            }
    ...
    public Player create(Player obj) {
        try{
             ResultSet result = this.connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT * FROM player");
             if (result.first()){
